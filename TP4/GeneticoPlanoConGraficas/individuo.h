...
public:
	...
	void returnValor(float returnXY[]){
		...
		float aux=0;
		int cantBitesEntero=6;
		if(iniY<cantBitesEntero)
			cantBitesEntero=iniY-1;
		
		for(int i=1;i<cantBitesEntero+1;i++){
			aux+=(cromosoma[i]*pow(2,cantBitesEntero-i));
		}
		//****************** Parte decimal *******************
		for(int i=cantBitesEntero+1;i<iniY;i++){
			aux+=(cromosoma[i]*pow(2,cantBitesEntero-i));
		}
		if(cromosoma[1]==1)
			aux*=-1;
		...
		for(int i=iniY+1;i<cantBitesEntero+iniY+1;i++){
			aux+=(cromosoma[i]*pow(2,cantBitesEntero+iniY-i));
		}
		//****************** Parte decimal *******************
		for(int i=cantBitesEntero+iniY+1;i<size;i++){
			aux+=(cromosoma[i]*pow(2,cantBitesEntero+iniY-i));
		}
		if(cromosoma[iniY]==1)
			aux*=-1;
