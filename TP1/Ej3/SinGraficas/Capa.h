...
//		cout<<"cantNeu "<<cantNeu<<"  canEntra  "<<canEntra<<endl;
		for(int i=0; i<cantNeu;i++){
			for(int j=0;j<canEntra;j++){
//				cout<<"Peso viejo:  "<<w[i][j]<<endl;
//				cout<<"deltaiente:  "<<delta[i]<<endl;
				w[i][j] = w[i][j] + nu*delta[i] * x[j];
//				w[i][j] = w[i][j] + (nu*delta[i] * x[j] + dw[i][j]);
//				dw[i][j]+= nu*delta[i] * x[j];
				
//				cout<<"Peso NUEVO:  "<<w[i][j]<<endl;
			}
//			cout<<endl;
		}
		x.clear();
