function notasXventana = preprocessor(nom)
    % ------------------------------------------------------
    % Preprocesa archivos midi, con ventaneo temporal
    % ------------------------------------------------------
    % ENTRADA
    % nom = nombre del archivo midi
    % ------------------------------------------------------
    % SALIDA
    % notasXventana = matriz con la cantidad de notas por ventana
    % ------------------------------------------------------
    
    % carga el archivo midi
    midi_data=readmidi([nom,'.mid']);
    ...
    csvwrite(['archivo-',nom,'.csv'],notasXventana);
    
    % grafica
    % calcula el piano-roll
    [PR,t,nn]=piano_roll(notas);
    
    % grafica el piano-roll
    figure;
    imagesc(t,nn,PR);
    axis xy;
    title(['MIDI original - ',nom]);
    xlabel('Tiempo (s)');
